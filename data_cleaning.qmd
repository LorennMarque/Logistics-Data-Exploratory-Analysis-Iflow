![](banner.png){width="770" height="215"}
...
raw_data <- read_csv("data/raw_iflow_data.csv")

# Configuracion de claves de API
...
Sys.setlocale("LC_TIME", "es_ES.UTF-8")  # Para Unix/macOS
# En Windows: Sys.setlocale("LC_TIME", "Spanish_Spain.1252")
...
```{r}
write.csv(x = raw_data, file = "iflow_clean.csv")
```
...
### Intentamos identificar algun patron en las entregas segun el día de la semana
...
plot_ly(distribucion_categorica, 
  ...
  layout(title = "Distribución de Entregas por Día de la Semana",
...
raw_data$dia_str <- factor(raw_data$dia_str, 
...
# Asumimos que tu DataFrame ya está cargado y se llama 'datos'
...
ggplot(resumen, aes(x = hora, y = dia_str, fill = n)) +
  ...
  labs(title = "Número de usos por día y hora", 
       x = "Hora del día", 
       y = "Día de la semana", 
       fill = "Cantidad") +
  theme_minimal() +
